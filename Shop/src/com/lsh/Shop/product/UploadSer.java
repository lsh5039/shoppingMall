...
public class UploadSer extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int maxSize =1024*1024*5;//5메가까지 저장가능
		...
		///////////enctype="multipart/form-data 전송은 request.getparameter불가 
...
//		System.out.println("파일명 : "+pd.getP_file());
//		System.out.println("실제 파일명 : "+pd.getP_realfile());
		...
		if(result==1) {//정상
			writer.println("alert('상품등록을 완료했습니다')");
			writer.println("location.href='/upload/product'");
		} else if(result==-1) {//비정상
			writer.println("alert('상품등록에 실패했습니다.')");
			writer.println("history.back()");
		}else if(result == 0) {
			writer.println("alert('데이터베이스 오류가 발새했습니다.')");
			writer.println("history.back()");
		}
		writer.println("</script>");
	}
